...
const SECURITY_RULES = [
  {
    ...
    severity: 'HIGH',
    regex:
      /(password\s*===?\s*['"](admin|root|password|master|123456|pass|secret|bypass)['"]|bypassAuth\s*[:=]\s*true|isBackdoor\s*[:=]|skipAuth\s*[:=]\s*true)/i,
  ...
  },
  {
    ...
    severity: 'HIGH',
    regex:
      /(JWT_SECRET\s*=\s*['"][^'"]{3,}['"]|AWS_SECRET_ACCESS_KEY\s*=\s*['"][^'"]{5,}['"]|DATABASE_URL\s*=\s*['"]postgres:\/\/[^'"]+['"]|api_key\s*:\s*['"]sk_[^'"]+['"])/i,
  ...
  },
  {
    ...
    severity: 'HIGH',
    regex:
      /(\beval\s*\(|new\s+Function\s*\(|child_process\.(exec|spawn)|require\(['"]child_process['"]\))/g,
  ...
  },
  {
    ...
    severity: 'HIGH',
    regex:
      /(app|router)\.(get|post|put|delete|use)\s*\(\s*['"]\/(backdoor|debug-shell|dump-db|admin-override|root-access|exec-cmd)['"]/i,
...
function isIgnored(filepath) {
  ...
  return IGNORED_PATHS.some(
    (ignored) => relative.startsWith(ignored) || relative.includes(`${path.sep}${ignored}`)
  );
}
...
if (totalViolationsFound === 0) {
  ...
  console.log(
    `🚨 Security Scan FAILED: Found ${totalViolationsFound} potential security violation(s)!\n`
  );
  ...
  console.log(
    '❌ Action Required: Remove backdoors, master credentials, or dangerous code before committing.'
  );
  process.exit(1);
}
